...
export default function SentinelsFullFight3D({ trainingDifficulty, keyBindings, hudSettings, cameraSettings, onCameraSettingsChange, onExit }: EncounterRuntimeProps) {
  ...
  return <main className="training-shell sentinels-runtime">
    ...
    </div><p className="train3d-controls">Move {keyLabel(keyBindings.forward)} {keyLabel(keyBindings.left)} {keyLabel(keyBindings.backward)} {keyLabel(keyBindings.right)} · turn {keyLabel(keyBindings.turnLeft)} {keyLabel(keyBindings.turnRight)} · Main {keyLabel(keyBindings.mainAbility)}{selected.role === 'healer' ? ` · Dispel ${keyLabel(keyBindings.dispel)}` : ''} · left orbit · right face · both buttons forward</p></div></section>
