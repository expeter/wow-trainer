...
function stepActive(state: SentinelsState, commands: PlayerCommandState, seconds: number, screenRelative = false): SentinelsState {
  ...
  if (next.dropletsSpawned) {
    let droplets = next.droplets.map(droplet => {
      ...
      const spawnAge = next.time - (next.dropletsSpawnedAt ?? next.time)
      if ((!droplet.assignedToPlayer || next.assignedSide !== droplet.side) && spawnAge >= 4 + Number(droplet.id.at(-1)) * .25) return { ...droplet, soaked: true, soakedAt: next.time }
      return droplet
    })
    next = { ...next, droplets }
    ...
    const beam = droplets.find(droplet => droplet.soakedAt !== undefined && next.time - droplet.soakedAt > .25 && next.time - droplet.soakedAt < timing.livingVenomSeconds)
  ...
  if (!next.miasmaResolved && phaseAge >= miasmaStart + timing.miasmaSeconds) {
    ...
    next = { ...next, miasmaResolved: true, puddleDropAt: next.assignedSide === 'blood' ? next.time + 6 : undefined }
  }
  ...
  if (phaseAge >= 40 && !next.protovenomResolved && !next.protovenomActive) {
    ...
    next = { ...next, protovenomActive: true, protovenomCarrierIds: carriers }
  }
  if (next.protovenomActive && !next.protovenomResolved) {
    ...
    } else if (nearby) next = { ...next, protovenomResolved: true, protovenomActive: false }
  }
...
function memberPosition(member: ContractRaidMember, state: SentinelsState): WorldPoint {
  ...
  const returnBeam = state.droplets.find(droplet => droplet.soakedAt !== undefined && state.time - droplet.soakedAt > .2 && state.time - droplet.soakedAt < sentinelsTiming(state.projection).livingVenomSeconds)
...
export function sentinelsSnapshot(state: SentinelsState): Train3DSnapshot {
  ...
  const actors: ActorSnapshot[] = contractRosterForSlot(state.selectedSlotId).map(member => ({
    ...
    auras: state.phase === 'stasis' && phaseAge >= 2 && (!state.helicalResolved || (!member.controlled && state.helicalResolvedAt !== undefined && state.time - state.helicalResolvedAt < 3)) ? (member.controlled ? toxins(1, 3) : (() => { const toxin = toxinComposition(contractRaidRoster.indexOf(member)); return toxins(toxin.green, toxin.red) })()) : member.controlled ? [
  ...
  for (const droplet of state.droplets) {
    ...
    if (droplet.soakedAt !== undefined && state.time - droplet.soakedAt < timing.livingVenomSeconds) effects.push({ id: `${droplet.id}-return`, kind: 'projectile', position: droplet.position, target: state.acidBoss, radius: 1.2, color: '#83ff68', progress: Math.min(1, (state.time - droplet.soakedAt) / timing.livingVenomSeconds) })
  }
  ...
  if (state.protovenomActive) {
    ...
    actors.filter(actor => state.protovenomCarrierIds.includes(actor.id)).forEach(actor => effects.push({ id: `${actor.id}-protovenom-ring`, kind: 'ground-spread', position: actor.position, radius: 3, color: '#ff4059', progress: 1, filled: false }))
  }
