...
export default function SentinelsFullFight2D({ trainingDifficulty, keyBindings, onExit }: EncounterRuntimeProps) {
  ...
  return <main className="training-shell sentinels-runtime">
    ...
    <section className="training-runtime-layout arena-only"><div className="learn2d-stage"><div className="learn2d-arena-frame"><div className="learn2d-board sentinels-2d-board" aria-label="Entombed Sentinels raid-plan training arena" style={{ '--sentinels-plan': `url(${RAID_PLAN})` } as CSSProperties}>
      ...
      {snapshot.effects.filter(effect => effect.kind !== 'cosmetic-projectile').map(effect => <div key={effect.id} className={`nekzali-2d-effect ${effect.kind === 'ground-soak' ? 'soak' : effect.kind === 'ground-spread' ? 'spread' : 'harmful'}${effect.filled === false ? ' outline' : ''}`} style={{ left: `${xPercent(effect.position.x)}%`, top: `${zPercent(effect.position.z)}%`, width: `${effect.radius / sentinelsArena.width * 200}%`, aspectRatio: '1', '--effect-color': effect.color } as CSSProperties} />)}
      ...
      {snapshot.actors.filter(actor => actor.kind === 'ally').map(actor => { const member = roster.find(item => item.id === actor.id); const role = member?.role ?? 'ranged'; return <div key={actor.id} className={`contract-raid-member ${role}`} style={{ left: `${xPercent(actor.position.x)}%`, top: `${zPercent(actor.position.z)}%` }} aria-label={member ? `${role} NPC` : `${actor.id.replaceAll('-', ' ')} NPC`}><span />{actor.auras.length > 0 && <ToxinIcons green={actor.auras.find(aura => aura.tone === 'poison')?.stacks ?? 0} red={actor.auras.find(aura => aura.tone === 'danger')?.stacks ?? 0} />}</div> })}
