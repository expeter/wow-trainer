...
import { activeSentinelsPrompt, createSentinelsState, dispelSentinels, nextSentinelsTimer, prepareSentinelsSlot, sentinelsPlayerRole, sentinelsSnapshot, stepSentinelsState } from '../simulation'
...
export default function SentinelsFullFight2D({ scenarioId, trainingDifficulty, keyBindings, onExit }: EncounterRuntimeProps) {
  ...
  function retry() { stateRef.current = createSentinelsState(gate.selectedSlotId, trainingDifficulty, mythic); setView(stateRef.current) }
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      if (gate.phaseRef.current === 'active' && !pause.pausedRef.current) stateRef.current = stepSentinelsState(stateRef.current, commandsRef.current, seconds)
  ...
  return <main className="training-shell sentinels-runtime">
    ...
    </div></div><div className="learn2d-controls"><span>Move {keyLabel(keyBindings.forward)} {keyLabel(keyBindings.left)} {keyLabel(keyBindings.backward)} {keyLabel(keyBindings.right)}{selected.role === 'healer' ? ` · Dispel ${keyLabel(keyBindings.dispel)}` : ''}</span>{selected.role === 'healer' && <button type="button" onClick={dispel} disabled={!view.blightedActive || view.blightedResolved}>Dispel</button>}</div></div></section>
  </main>
}
