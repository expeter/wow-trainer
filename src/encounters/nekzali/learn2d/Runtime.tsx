...
import { activeNekzaliPrompt, createNekzaliState, interruptNekzali, nekzaliSnapshot, nextNekzaliTimer, prepareNekzaliSlot, startNekzaliMainCast, stepNekzaliState, tauntNekzali } from '../simulation'
...
export default function NekzaliLearn2D({ scenarioId, trainingDifficulty, keyBindings, onExit }: EncounterRuntimeProps) {
  ...
  function retry() { stateRef.current = createNekzaliState(gate.selectedSlotId, trainingDifficulty, mythic); setView(stateRef.current) }
  ...
  useEffect(() => {
    ...
    const down = (event: KeyboardEvent) => { if (!event.repeat && gate.phaseRef.current === 'active') { if (event.code === keyBindings.mainAbility) stateRef.current = startNekzaliMainCast(stateRef.current); if (event.code === keyBindings.taunt) stateRef.current = tauntNekzali(stateRef.current); if (event.code === keyBindings.interrupt) stateRef.current = interruptNekzali(stateRef.current) } update(event, true) }
  ...
  return <main className="training-shell nekzali-runtime">
    <RuntimeStatusBar meta={`NEK'ZALI · ${mythic ? 'MYTHIC' : 'HEROIC'} RULES · ${trainingDifficulty.toUpperCase()} TRAINER · ${selected.role.toUpperCase()}`} title="Nek'zali tactical full fight" status={`${view.realmStage !== 'none' ? 'WELL REALM' : phaseLabel} · ${activeNekzaliPrompt(view)}`} paused={pause.paused} pauseKey={keyBindings.pause} onTogglePause={pause.toggle} onExit={onExit} />
    <section className="training-runtime-layout arena-only"><div className="learn2d-stage"><div className="learn2d-arena-frame"><div className={`learn2d-board nekzali-2d-board${view.realmStage !== 'none' ? ' realm-active' : ''}`} aria-label="Nek'zali raid-plan training arena" style={{ '--nekzali-plan': `url(${RAID_PLAN})` } as CSSProperties}>
      ...
      {snapshot.actors.filter(actor => actor.kind === 'ally').map(actor => <div key={actor.id} className="contract-raid-member ranged" style={{ left: `${percent(actor.position.x)}%`, top: `${percent(actor.position.z)}%` }} aria-label="assigned-realm NPC"><span /></div>)}
