...
export default function NekzaliLearn2D({ trainingDifficulty, keyBindings, onExit }: EncounterRuntimeProps) {
  ...
  const effectClass = (kind: string) => kind === 'ground-soak' ? 'soak' : kind === 'ground-spread' ? 'spread' : kind === 'arrow' ? 'arrow' : kind.includes('projectile') ? 'projectile' : kind === 'dome' ? 'dome' : 'harmful'
  ...
  return <main className="training-shell nekzali-runtime">
    ...
    <section className="training-runtime-layout arena-only"><div className="learn2d-stage"><div className="learn2d-arena-frame"><div className={`learn2d-board nekzali-2d-board${view.realmStage !== 'none' ? ' realm-active' : ''}`} aria-label="Nek'zali raid-plan training arena" style={{ '--nekzali-plan': `url(${RAID_PLAN})` } as CSSProperties}>
      ...
      {snapshot.effects.map(effect => {
        const rendered = effect.target ? { x: effect.position.x + (effect.target.x - effect.position.x) * effect.progress, z: effect.position.z + (effect.target.z - effect.position.z) * effect.progress } : effect.position
        return <div key={effect.id} data-effect-kind={effect.kind} data-projectile-shape={effect.projectileShape} className={`nekzali-2d-effect ${effectClass(effect.kind)}${effect.filled === false ? ' outline' : ''}`} style={{ left: `${percent(rendered.x)}%`, top: `${percent(rendered.z)}%`, width: `${effect.radius / 90 * 200}%`, aspectRatio: '1', '--effect-color': effect.color, '--effect-rotation': effect.target ? `${Math.atan2(effect.target.z - effect.position.z, effect.target.x - effect.position.x)}rad` : '0rad' } as CSSProperties}>{effect.kind === 'arrow' ? '➜' : ''}</div>
      })}
