...
import { EncounterCastBars } from '../../../platform/TrainingHud'
...
export default function NekzaliLearn2D({ trainingDifficulty, keyBindings, hudSettings, onExit }: EncounterRuntimeProps) {
  ...
  return <main className="training-shell nekzali-runtime">
    ...
    <section className="training-runtime-layout arena-only"><div className="learn2d-stage"><div className="learn2d-arena-frame"><div className={`learn2d-board nekzali-2d-board${view.realmStage !== 'none' ? ' realm-active' : ''}`} aria-label="Nek'zali raid-plan training arena" style={{ '--nekzali-plan': `url(${RAID_PLAN})` } as CSSProperties}>
      ...
      <EncounterCastBars settings={hudSettings} castSeconds={view.mainCastRemaining} castSecondsSource={() => stateRef.current.mainCastRemaining} enemyCast={view.innerCastStartedAt !== undefined && !view.innerCastInterrupted ? { label: 'Drowned Echo · Soulcoil', seconds: Math.max(0, 5 - (view.time - view.innerCastStartedAt)), duration: 5 } : undefined} />
