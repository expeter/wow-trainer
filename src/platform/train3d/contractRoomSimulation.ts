...
import { auraToneColors, contractDirections, contractRaidRoster, contractTones, CONTRACT_EVENT_SECONDS, CONTRACT_LANDING_SECONDS, seededContractEvents, type ContractDirection, type ContractEvent, type ContractRaidMember } from '../contractRoom'
...
const contractGroundSlots: Record<ContractDirection, WorldPoint> = {
  north: { x: 0, z: -18 }, east: { x: 25, z: 0 }, south: { x: 0, z: 18 }, west: { x: -25, z: 0 },
}
...
export const contractRoomArena = {
  id: 'platform_contract_room', label: 'Platform contract room', shape: 'rectangle', width: 64, depth: 48,
...
function worldPosition(member: ContractRaidMember): WorldPoint {
  if (member.controlled) return { x: 0, z: 8 }
...
export function contractRoomSnapshot(state: ContractRoomState): Train3DSnapshot {
  ...
  const npcActors = contractRaidRoster.filter(member => !member.controlled).map((member, index) => {
    ...
    return {
      ...
      auras: index % 4 === 0 ? [{ id: `npc-${tone}`, tone, stacks: 1 }] : [],
    }
  })
  return {
    time: state.time,
    ...
    actors: [
      { id: 'player', kind: 'player', position: state.player, facing: state.player.facing, color: '#f2d36b', auras: [{ id: event.id, tone: event.tone, stacks: 1 }] },
