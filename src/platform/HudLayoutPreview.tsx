...
export default function HudLayoutPreview({ settings, onChange }: { settings: TrainingHudSettings; onChange: (settings: TrainingHudSettings) => void }) {
  ...
  function setPoint(box: HudBox, x: number, y: number) {
  ...
  function move(box: HudBox, event: PointerEvent<HTMLButtonElement>) {
    ...
    setPoint(box, (event.clientX - bounds.left) / bounds.width * 100, (event.clientY - bounds.top) / bounds.height * 100)
  }
  ...
  return <div className="hud-layout-editor">
    <div className="hud-layout-preview" ref={stageRef} aria-label="Draggable HUD preview">
      {(Object.keys(labels) as HudBox[]).filter(box => visible[box]).map(box => <button type="button" key={box} className={`hud-preview-box ${box}`} style={{ left: `${draft[box].x}%`, top: `${draft[box].y}%` }} onPointerDown={event => { event.preventDefault(); draggingRef.current = box; event.currentTarget.setPointerCapture(event.pointerId) }} onPointerMove={event => move(box, event)} onPointerUp={event => finish(box, event)} onPointerCancel={event => finish(box, event)} onKeyDown={event => nudge(box, event)}>{labels[box]}</button>)}
    </div>
